...
const OffersMade = (props) => {
    ...
    const loadOffers = async _listing => {
        const _query = query(collection(db, constants.dbOffers), where("user_lowerCase", "==", props.account.address.toLowerCase()));
    ...
    const columns = [
        ...
        {
            name: "Status",
            selector: (row) => row.expiry > Date.now() && !row.cancelled && !row.fulfilled ? "Active" : "In Active",
        },
        {
            name: "Details",
            center: true,
